...
function getRectangleString(width, height) {
  ...
  const interimStr = `│${' '.repeat(width - 2)}│\n`;
  return firstStr + interimStr.repeat(height - 2) + lastStr;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Za-z]/g, (match) => {
    if (match.charCodeAt() >= 65 && match.charCodeAt() <= 90) {
    ...
    } else if (match.charCodeAt() >= 97 && match.charCodeAt() <= 122) {
